...
static inline void lowrisc_writeq(struct lowrisc_host *host, int reg, u64 val)
{
	writeq(val, host->addr + (reg << host->bus_shift));
}

static inline u64 lowrisc_readq(struct lowrisc_host *host, int reg)
{
	return readq(host->addr + (reg << host->bus_shift));
}

static inline void lowrisc_writew(struct lowrisc_host *host, int reg, u16 val)
{
	writew(val, host->addr + (reg << host->bus_shift));
}

static inline u16 lowrisc_readw(struct lowrisc_host *host, int reg)
{
	return readw(host->addr + (reg << host->bus_shift));
}

static void lowrisc_detect(struct lowrisc_host *host)
{
...
{
	struct lowrisc_host *host = dev_id;
...
static int lowrisc_wait_interrupt_flag(struct lowrisc_host *host)
{
...
static int lowrisc_clock_control(struct lowrisc_host *host, unsigned long clk)
{
...
static int lowrisc_sync_reset(struct lowrisc_host *host)
{
	u32 timeout;
...
static int lowrisc_error_manage(struct lowrisc_host *host)
{
...
static int lowrisc_single_read(struct lowrisc_host *host, struct mmc_data *data)
{
	long time;
...
static int lowrisc_multi_read(struct lowrisc_host *host, struct mmc_data *data)
{
	long time;
...
static int lowrisc_single_write(struct lowrisc_host *host,
...
static int lowrisc_multi_write(struct lowrisc_host *host, struct mmc_data *data)
{
	long time;
...
static void lowrisc_get_response(struct lowrisc_host *host, struct mmc_cmd *cmd)
{
...
static unsigned short lowrisc_set_cmd(struct lowrisc_host *host,
...
static unsigned short lowrisc_data_trans(struct lowrisc_host *host,
			struct mmc_data *data, unsigned short opc)
{
	if (host->app_cmd) {
		host->app_cmd = 0;
...
static int lowrisc_start_cmd(struct lowrisc_host *host,
...
static int lowrisc_send_cmd_common(struct lowrisc_host *host,
...
static int lowrisc_set_ios_common(struct lowrisc_host *host, struct mmc *mmc)
{
	int ret;
...
static int lowrisc_initialize_common(struct lowrisc_host *host)
{
...
#if defined(__BIG_ENDIAN_BITFIELD)
	lowrisc_writew(host, SDHI_EXT_SWAP, SET_SWAP);
#endif
...
static int lowrisc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd,
			    struct mmc_data *data)
{
	struct lowrisc_host *host = mmc_priv(mmc);
...
int lowrisc_init(unsigned long addr, int ch, unsigned long quirks)
{
	int ret = 0;
	struct mmc *mmc;
	struct lowrisc_host *host = NULL;

	if (ch >= CONFIG_SYS_SH_SDHI_NR_CHANNEL)
		return -ENODEV;

	host = malloc(sizeof(struct lowrisc_host));
	if (!host)
		return -ENOMEM;
...
int lowrisc_dm_send_cmd(struct udevice *dev, struct mmc_cmd *cmd,
			struct mmc_data *data)
{
	struct lowrisc_host *host = dev_get_priv(dev);

	return lowrisc_send_cmd_common(host, cmd, data);
}

int lowrisc_dm_set_ios(struct udevice *dev)
{
	struct lowrisc_host *host = dev_get_priv(dev);
...
static const struct dm_mmc_ops lowrisc_dm_ops = {
	.send_cmd	= lowrisc_dm_send_cmd,
	.set_ios	= lowrisc_dm_set_ios,
};

static int lowrisc_dm_bind(struct udevice *dev)
{
	struct lowrisc_plat *plat = dev_get_platdata(dev);

	return mmc_bind(dev, &plat->mmc, &plat->cfg);
}

static int lowrisc_dm_probe(struct udevice *dev)
{
	struct lowrisc_plat *plat = dev_get_platdata(dev);
	struct lowrisc_host *host = dev_get_priv(dev);
	struct mmc_uclass_priv *upriv = dev_get_uclass_priv(dev);
	struct clk lowrisc_clk;
	...
	ret = clk_get_by_index(dev, 0, &lowrisc_clk);
	if (ret) {
		debug("failed to get clock, ret=%d\n", ret);
		return ret;
	}

	ret = clk_enable(&lowrisc_clk);
	if (ret) {
	...
	lowrisc_initialize_common(host);
...
static const struct udevice_id lowrisc_sd_match[] = {
	{ .compatible = "renesas,sdhi-r8a7795", .data = SH_SDHI_QUIRK_64BIT_BUF },
	{ .compatible = "renesas,sdhi-r8a7796", .data = SH_SDHI_QUIRK_64BIT_BUF },
	{ /* sentinel */ }
};

U_BOOT_DRIVER(lowrisc_mmc) = {
	.name			= "sh-sdhi-mmc",
	.id			= UCLASS_MMC,
	.of_match		= lowrisc_sd_match,
	.bind			= lowrisc_dm_bind,
	.probe			= lowrisc_dm_probe,
	.priv_auto_alloc_size	= sizeof(struct lowrisc_host),
	.platdata_auto_alloc_size = sizeof(struct lowrisc_plat),
	.ops			= &lowrisc_dm_ops,
};
#endif
